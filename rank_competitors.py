...
and NYC North Regional 2025-26.
"""
...
def get_competitor_placements():
    ...
    competitor_placements = defaultdict(list)
    competitor_events = defaultdict(list)
...
def calculate_score(placements):
    """Calculate average placement (lower is better)"""
    if not placements:
        return float('inf')
...
for name, data in competitor_data.items():
    if data["placements"]:
        avg_placement = calculate_score(data["placements"])
...
# Sort by average placement (lower is better), then by number of events (more is better)
ranked_competitors.sort(key=lambda x: (x["avg_placement"], -x["num_events"]))

# Print top 20
print("=" * 100)
print("TOP 20 STRONGEST COMPETITORS")
print("Based on average event placements in Yale Invitational 2025-26 and NYC North Regional 2025-26")
print("=" * 100)
print()

for rank, comp in enumerate(ranked_competitors[:20], 1):
    placements_str = ", ".join(map(str, sorted(comp["placements"])))
    print(f"{rank:2d}. {comp['name']:25s} | Avg: {comp['avg_placement']:5.2f} | "
          f"Events: {comp['num_events']:2d} | Competitions: {comp['num_competitions']} | "
